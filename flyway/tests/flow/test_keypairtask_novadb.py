...
from flyway.flow.keypairtask_nova_db import KeypairNovaDBMigrationTask
from flyway.flow.update_keypair_user_task import UpdateKeypairUserTask
from utils.db_handlers import keypairs
from utils.helper import *


class KeypairTaskNovaDBTest(TestBase):

    def __init__(self, *args, **kwargs):
        super(KeypairTaskNovaDBTest, self).__init__(*args, **kwargs)
        config.parse(['--config-file', '../../etc/flyway.conf'])
        self.migration_task = \
            KeypairNovaDBMigrationTask('keypair_migration_task')
