...
from flyway.utils.helper import generate_new_password
...
class SampleApp():
    ...
    def startMigration(self):
        ...
        for name in self.res_toMigrate.keys():
            ...
            for element in self.res_toMigrate[name]:
                self.resourceSum[name][element] = self.originalRes[name][
                    element]
        """
    ...
    def sourcePanel(self):
        ...
        for i, text in enumerate(self.buttonTexts):
            self.s_buttons[text] = Button(self.m1, text=text,
                                          command=self.s_actions[text])
        ...
        self.sl2.pack()
        self.start = Button(self.m4, text='Start Migration', bg='orange',
                            command=self.startMigration)
    ...
    def targetPanel(self):
        ...
        for i, text in enumerate(self.buttonTexts):
            self.t_buttons[text] = Button(self.n1, text=text,
                                          command=self.t_actions[text])
        ...
        self.tback = Button(self.n4, text='New Migration', bg='lightblue',
                            command=self.showCloudSelection)
        self.tback.pack()
    ...
    def getOriginalResource(self):
        self.s_buttons = {}
        self.t_buttons = {}
        self.buttonTexts = ['All', 'Projects', 'Images', 'Keypairs', 'Users',
                            'Roles', 'Instances']
    ...
    def migrateImages(self, images):
        ...
        for source_image in images.keys():
            ...
            if image_checksum not in target_imageChecksums:
                ...
                image_data = self.s_glance.images.data(image=image_id,
                                                       do_checksum=True)
...
class cloudsSelection():
    def __init__(self, **kwargs):
        ...
        self.b1 = Button(self.m1, text='Source', bg='green',
                         command=self.sourceChosen)
        ...
        self.submit = Button(self.m1, text='Submit', bg='orange',
                             command=self.submit, width=22)
        self.submit.pack()
    ...
    def __init__(self, source, target, *args, **kwargs):
        ...
        if source != 'None':
            ...
            for text in self.texts:
                ...
                if text == 'password':
                    self.entries1[text] = Entry(self.m, bd=3, show='*',
                                                width=40)
        ...
        if target != 'None':
            ...
            for text in self.texts:
                ...
                if text == 'password':
                    self.entries2[text] = Entry(self.m, bd=3, show='*',
                                                width=40)
        ...
        self.submit = Button(self.m1, text='Submit', bg='orange', width=15,
                             command=self.showResourceWindow)
        self.submit.pack()

        self.back = Button(self.m1, text='Return', bg='orange',
                           command=self.goBack)
        self.back.pack()
...
class welcomeWindow():
    def __init__(self):
        ...
        self.button = Button(self.m1, text='Let\'s roll!', command=self.start,
                             bg='red', width=30)
        self.button.pack()
        self.exit = Button(self.m1, text='Bye!', command=self.end,
                           bg='lightblue')
        self.exit.pack()
